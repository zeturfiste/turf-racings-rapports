...
def main():
    ...
    problems_total = 0
    global_summary = []               # (date, any_missing, any_extra)
    per_year_summary: dict[str, list] = {}

    for ddir in iter_date_dirs(root):
        date_str = ddir.name          # YYYY-MM-DD
        year_str = date_str[:4]

        year_outdir = outdir / year_str
        ...
        for rslug in expected_reunions:
            ...
            present_courses = {
                p.name
                for p in rdir.glob("R*C*-*.html")
                if p.name != f"{rslug}.html"
            }
        ...
        present_rdirs = {
            ...
            if p.is_dir() and REUNION_DIR_RE.match(p.name)
        }
        extras_r = sorted(present_rdirs - expected_set)
        for exr in extras_r:
            report["extras"]["reunion_dirs"].append(str(ddir / exr))

        # --- Sauvegarde JSON par date (dans missing/YYYY/) ---
        (year_outdir / f"{date_str}.json").write_text(
        ...
        any_missing = (
            miss["date_html"] or
            miss["reunion_dirs"] or
            miss["reunion_html"] or
            miss["course_files"]
        )
        any_extra = report["extras"]["reunion_dirs"] or report["extras"]["course_files"]

        global_summary.append((date_str, any_missing, any_extra))
        per_year_summary.setdefault(year_str, []).append((date_str, any_missing, any_extra))
    ...
    if not global_summary:
        ...
        lines.append("|------|-----------|--------|")
        for date_str, any_missing, any_extra in sorted(global_summary):
            lines.append(
                f"| {date_str} | {'❌' if any_missing else '✅'} | {'⚠️' if any_extra else '—'} |"
            )
    ...
    for year_str, rows in per_year_summary.items():
        ...
        ylines.append("| Date | Manquants | Extras |")
        ylines.append("|------|-----------|--------|")
        for date_str, any_missing, any_extra in sorted(rows):
            ylines.append(
                f"| {date_str} | {'❌' if any_missing else '✅'} | {'⚠️' if any_extra else '—'} |"
            )
        (outdir / year_str / "summary.md").write_text("\n".join(ylines), encoding="utf-8")
